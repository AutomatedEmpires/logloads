...
//
// Reads the operating-state mirror (service-role only) and reports each org's plan
...
// filter. Requires the snapshot mirror to be enabled (SUPABASE_SERVICE_ROLE_KEY).
const url = process.env.SUPABASE_URL
const key = process.env.SUPABASE_SERVICE_ROLE_KEY
const orgFilter = process.argv[2]?.toLowerCase()

if (!url || !key) {
  console.error("Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY (server-only). The mirror is service-role only.")
  process.exit(2)
}

const res = await fetch(`${url}/rest/v1/operating_state?id=eq.primary&select=state,updated_at`, {
...
if (!state) {
  console.error("No operating_state row. Mirror may be empty (single-node has not persisted yet).")
  process.exit(1)
}
...
console.log(`Mirror updated: ${rows[0].updated_at}`)

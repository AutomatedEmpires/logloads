...
await page.waitForURL("**/driver/map", { timeout: 30000 }).catch(() => console.log("sign-in stuck at", page.url()))
...
for (let index = 0; index < count; index += 1) {
  ...
  const requestable = await page.getByRole("button", { name: "Request haul" }).isVisible().catch(() => false)
  const committed = await page.getByText("The host is deciding.").first().isVisible().catch(() => false)
  ...
  if (requestable) {
    ...
    const ok = await page.getByText("Request sent").first().isVisible().catch(() => false)

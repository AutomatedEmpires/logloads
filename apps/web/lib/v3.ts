...
export type V3Role = keyof typeof V3_ACTORS
...
export function getRoleNetwork(role: V3Role): NetworkView {
  return buildNetworkView(services.state, V3_ACTORS[role])
}

export function getDriverNetwork(): NetworkView {
  return getRoleNetwork("driver")
}

export function getFleetNetwork(): NetworkView {
  return getRoleNetwork("fleet")
}

export function getHostNetwork(): NetworkView {
  return getRoleNetwork("host")
}

export function getPublicLoads(): NetworkLoadView[] {
  return getDriverNetwork().loads.filter((load) =>
    ["open_network", "verified_network"].includes(load.visibilityMode) &&
...
export function getAdminSummary() {
  ...
  return {
    ...
    verificationRecords: state.verificationRecords.length,
    ...
    billingExceptions: state.entitlements.filter((entitlement) => !["active", "trialing"].includes(entitlement.status)).length
  }
}

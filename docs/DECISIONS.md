...
- Snapshot durability is two-tier: local disk is primary; a Supabase-backed mirror (`operating_state` table, migration `20260706210000_operating_state_mirror.sql`) is written on every persisted mutation and restored ONLY when a fresh node boots without a disk snapshot. Activates via `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` (or a server-only anon key). A dedicated LogLoads Supabase project could not be provisioned this pass: the AutomatedEmpires org is at its 2-active-free-project limit (sweepza, explore&earn) — freeing a slot or upgrading the org is a founder decision ($0/month once a slot exists).
